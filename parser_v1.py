...
class Parser:
    ...
    def parse_motif_line(self):
        # motif_line → element motif_line | epsilon
        t = self.lexer.peek(1)
        if t.token_type in {TokenType.KNIT, 
                            TokenType.PURL, 
                            TokenType.KFB, 
    ...
    def parse_element(self):
        ...
        elif t.token_type == TokenType.FILL:
            return
        else:
            self.syntax_error()
    ...
    def parse_expr_tail(self):
        ...
        if t.token_type in {TokenType.PLUS, TokenType.MINUS}:
            ...
            TokenType.MULT,    # <-- needed for motif_repeat: base_motif * expr
            TokenType.COMMA,   # <-- helpful for arg lists if you ever allow exprs there
            TokenType.FILL,    # <-- row_stmt: ... fill ;
            # also needed for stitch_count context: K expr <next element>
            TokenType.KNIT, TokenType.PURL, TokenType.KFB, TokenType.M1L, TokenType.M1R,
            TokenType.SSK, TokenType.K2TOG,
    ...
    def parse_term_tail(self):
        ...
        if t.token_type == TokenType.MULT:
            ...
            TokenType.KNIT, TokenType.PURL, TokenType.KFB, TokenType.M1L, TokenType.M1R,
            TokenType.SSK, TokenType.K2TOG,
            TokenType.ID, TokenType.LPAREN, TokenType.PLUS, TokenType.MINUS
        }:
